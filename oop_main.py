...
class HeatWave(GameObject):
    ...
    def interact(self, other):
        if isinstance(other, Player) or \
           isinstance(other, SoftWall):
               other.alive = False
...
class Game:
    ...
    def render(self):
        # w = max(go.x for go in self.game_objects) + 1
        # h = max(go.y for go in self.game_objects) + 1
        # field = [["." for __ in range(w)] for _ in range(h)]
        # for go in self.game_objects:
        #     field[go.y][go.x] = go.render()
        #
